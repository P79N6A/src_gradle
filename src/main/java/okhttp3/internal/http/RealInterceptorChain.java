        ...
        if (this.index < this.interceptors.size()) {
            ...
            if (this.httpCodec != null && !this.connection.supportsUrl(request2.url())) {
                ...
                if (httpCodec2 != null && this.index + 1 < this.interceptors.size() && realInterceptorChain.calls != 1) {
                    throw new IllegalStateException("network interceptor " + r3 + " must call proceed() exactly once");
                } else if (intercept == null) {
                    throw new NullPointerException("interceptor " + r3 + " returned null");
                } else if (intercept.body() != null) {
                    return intercept;
                } else {
                    throw new IllegalStateException("interceptor " + r3 + " returned a response with no body");
                }
            } else {
