...
public class r extends q {
    ...
    private static final <T> Iterator<T> iterator(@NotNull Iterator<? extends T> it2) {
        Intrinsics.checkParameterIsNotNull(it2, "$this$iterator");
        return it2;
    }

    @NotNull
    public static final <T> Iterator<ae<T>> withIndex(@NotNull Iterator<? extends T> it2) {
        Intrinsics.checkParameterIsNotNull(it2, "$this$withIndex");
        return new ag<>(it2);
    }

    public static final <T> void forEach(@NotNull Iterator<? extends T> it2, @NotNull Function1<? super T, Unit> function1) {
        Intrinsics.checkParameterIsNotNull(it2, "$this$forEach");
