...
public class LocaleUtils {
    ...
    public static Locale getUpdatedLocaleForAndroid(Locale locale) {
        String str = LANGUAGE_MAP_FOR_ANDROID.get(locale.getLanguage());
        if (str == null) {
            return locale;
        }
        return new Locale.Builder().setLocale(locale).setLanguage(str).build();
    }

    @VisibleForTesting
    @TargetApi(21)
    public static Locale getUpdatedLocaleForChromium(Locale locale) {
        String str = LANGUAGE_MAP_FOR_CHROMIUM.get(locale.getLanguage());
        if (str == null) {
            return locale;
        }
        return new Locale.Builder().setLocale(locale).setLanguage(str).build();
