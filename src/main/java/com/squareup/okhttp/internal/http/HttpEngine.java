    ...
    class NetworkInterceptorChain implements Interceptor.Chain {
        ...
        public Response proceed(Request request2) throws IOException {
            ...
            if (this.index < HttpEngine.this.client.networkInterceptors().size()) {
                ...
                Response intercept = HttpEngine.this.client.networkInterceptors().get(this.index).intercept(networkInterceptorChain);
                if (networkInterceptorChain.calls != 1) {
                    throw new IllegalStateException("network interceptor " + r7 + " must call proceed() exactly once");
                } else if (intercept != null) {
                    return intercept;
                } else {
                    throw new NullPointerException("network interceptor " + r7 + " returned null");
                }
            } else {
  ...
  ?: SGET   com.ss.android.ugc.aweme.shortvideo.sticker.ar.pixaloop.d.c.f int)*/:
    ...
    private static Headers combine(Headers headers, Headers headers2) throws IOException {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if ((!"Warning".equalsIgnoreCase(name) || !value.startsWith(PushConstants.PUSH_TYPE_THROUGH_MESSAGE)) && (!OkHeaders.isEndToEnd(name) || headers2.get(name) == null)) {
