...
public class CrashHandler implements Thread.UncaughtExceptionHandler {
    ...
    public Reporter obtainCommonReporter() {
        return new Reporter().addParams("app_id", Integer.valueOf(this.neverland.appId)).addParams("device_platform", "android").addParams("os_version_code", Integer.valueOf(Build.VERSION.SDK_INT)).addParams("device_type", Build.MODEL).addParams("device_fingerprint", Build.FINGERPRINT).addParams("sdk_int", Build.VERSION.RELEASE).addParams("random_id", this.neverland.randomId).addParams("device_id", this.neverland.deviceId).addParams("package_name", this.neverland.packageName).addParams("app_version", this.neverland.version).addParams("app_detail_version", this.neverland.detailVersion).addParams("is_first_launch", Boolean.valueOf(this.neverland.isFirstLaunch())).addParams("unfatal_exception_msg", this.neverland.unFatalExceptions).addParams("multidex_cost_time", Long.valueOf(this.neverland.costMultiDex));
    }
    ...
    private void handleJavaCrash(Thread thread, Throwable th) {
        ...
        File file2 = new File(file, System.currentTimeMillis() + ".jc");
