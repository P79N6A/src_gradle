...
public class RecorderManager implements VideoCapturer.VideoCapturerObserver, MediaEncodeStream.Observer, Transport.EventObserver, IRecorderManager {
    ...
    class WaterMarkProcess extends VideoProcessor implements SurfaceTexture.OnFrameAvailableListener {
    ...
    private void onStartRecorder() {
        ...
        if (z3) {
            ...
            if (start < 0) {
                ...
                iRecorderListener.onRecorderError(-7, new Exception("Start recorder error (" + start + ")"));
            }
        }
    }
                ...
                public void run() {
                    switch (i3) {
                        ...
                        case 3:
                            ...
                            IRecorderManager.IRecorderListener iRecorderListener = RecorderManager.this.mListener;
                            int i = i3;
                            iRecorderListener.onRecorderError(i, new Exception("Recorder have an internal error (" + i4 + ", " + j2 + ")"));
                            if (RecorderManager.this.mMediaEncodeStream != null) {
    ...
    public void start(String str, IRecorderManager.IRecorderListener iRecorderListener, IRecorderManager.Config config, int i) {
        if (this.mState.get() != 4) {
            ...
            final IRecorderManager.IRecorderListener iRecorderListener2 = iRecorderListener;
            final IRecorderManager.Config config2 = config;
    ...
    public void onStart(String str, IRecorderManager.IRecorderListener iRecorderListener, IRecorderManager.Config config, int i) {
        ...
        if (onUpdateConfig(config)) {
            ...
            } else if (!this.mConfig.havaVideo || !this.mConfig.haveAudio) {
                this.mState.set(3);
                iRecorderListener.onRecorderError(-6, new UnsupportedOperationException("Native recorder not supports."));
                return;
            } else {
