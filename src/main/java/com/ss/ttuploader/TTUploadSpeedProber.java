...
public class TTUploadSpeedProber implements TTVideoUploaderListener {
    ...
    private String getFilePath(Context context) {
        try {
            if (context.getApplicationContext().getFilesDir().getAbsolutePath() == null) {
                return null;
            }
            File file = new File(r3 + File.separator + "speedprober");
            if (!file.exists()) {
                file.mkdirs();
            }
            return r3 + File.separator + "speedprober.test";
