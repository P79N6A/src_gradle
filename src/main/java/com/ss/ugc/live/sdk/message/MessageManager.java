...
public class MessageManager extends HandlerThread implements IMessageManager {
    ...
    public void onLooperPrepared() {
        ...
        this.mSelfThreadHandler.sendEmptyMessage(BaseLoginOrRegisterActivity.o);
    }
    ...
    private void enqueueMessage(IMessage iMessage) {
        if (this.mEnablePriority) {
            ...
            while (true) {
                if (i >= size) {
                    break;
                } else if (iMessage.getPriority() >= this.mMessageQueue.get(i).getPriority()) {
    ...
    public void removeMessageListener(OnMessageListener onMessageListener) {
        if (onMessageListener != null) {
            ...
            for (int i = 0; i < size; i++) {
                Set valueAt = this.mMessageListenerMap.valueAt(i);
                if (valueAt != null) {
            ...
            public void onApiError(Exception exc) {
                ...
                if (MessageManager.this.mState == 2 && MessageManager.this.mSelfThreadHandler != null) {
                    ...
                    if (MessageManager.this.mFetchFailedCount > 3) {
                        ...
                        j = (long) (MessageManager.this.mFetchFailedCount * PushConstants.PUSHSERVICE_INFO_SENDMESSAGE_BY_NOTIFICATIONSERVICE);
                    }
                ...
                if (list != null && !list.isEmpty()) {
                    if (MessageManager.this.mIsAnchor) {
                        ...
                        for (IMessage next : list) {
                            if (next.needMonitor()) {
                                MessageManager.this.mMonitorMessageIds.add(Long.valueOf(next.getMessageId()));
                            }
                        }
        ...
        switch (message.what) {
            ...
            case 205:
                if (this.mState == 2) {
                    ...
                    for (IMessage next : this.mMessageQueue) {
                        if (next != null) {
                            onMessage(next);
                        }
                    }
    ...
    public void addMessageListener(int i, OnMessageListener onMessageListener) {
        if (onMessageListener != null) {
            Set set = this.mMessageListenerMap.get(i);
            if (set == null) {
