    ...
    class FrameDecodeRunnable implements Runnable {
        ...
        public void run() {
            try {
                if (this.mBitmapFrameCache.contains(this.mFrameNumber)) {
                    FLog.v(DefaultBitmapFramePreparer.TAG, "Frame %d is cached already.", (Object) Integer.valueOf(this.mFrameNumber));
                ...
                if (prepareFrameAndCache(this.mFrameNumber, 1)) {
                    FLog.v(DefaultBitmapFramePreparer.TAG, "Prepared frame frame %d.", (Object) Integer.valueOf(this.mFrameNumber));
                } else {
        ...
        private boolean prepareFrameAndCache(int i, int i2) {
            ...
            while (true) {
                switch (i2) {
                    ...
                    case 2:
                        try {
                            ...
                            FLog.w(DefaultBitmapFramePreparer.TAG, "Failed to create frame bitmap", (Throwable) e2);
                ...
                if (renderFrameAndCache || c2 == 65535) {
        ...
        private boolean renderFrameAndCache(int i, CloseableReference<Bitmap> closeableReference, int i2) {
            ...
            FLog.v(DefaultBitmapFramePreparer.TAG, "Frame %d ready.", (Object) Integer.valueOf(this.mFrameNumber));
            synchronized (DefaultBitmapFramePreparer.this.mPendingFrameDecodeJobs) {
    ...
    public boolean prepareFrame(BitmapFrameCache bitmapFrameCache, AnimationBackend animationBackend, int i) {
        ...
        synchronized (this.mPendingFrameDecodeJobs) {
            if (this.mPendingFrameDecodeJobs.get(uniqueId) != null) {
                FLog.v(TAG, "Already scheduled decode job for frame %d", (Object) Integer.valueOf(i));
                return true;
            } else if (bitmapFrameCache.contains(i)) {
                FLog.v(TAG, "Frame %d is cached already.", (Object) Integer.valueOf(i));
                return true;
            } else {
