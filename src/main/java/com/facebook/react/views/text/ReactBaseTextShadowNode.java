...
public abstract class ReactBaseTextShadowNode extends LayoutShadowNode {
    ...
    public void setLetterSpacing(float f2) {
        ...
        if (this.mAllowFontScaling) {
            f3 = PixelUtil.toPixelFromSP(this.mLetterSpacingInput);
        } else {
            f3 = PixelUtil.toPixelFromDIP(this.mLetterSpacingInput);
        }
    ...
    public void setFontSize(float f2) {
        ...
        if (f2 != -1.0f) {
            if (this.mAllowFontScaling) {
                f2 = (float) Math.ceil((double) PixelUtil.toPixelFromSP(f2));
            } else {
                f2 = (float) Math.ceil((double) PixelUtil.toPixelFromDIP(f2));
            }
        }
    ...
    private static void buildSpannedFromShadowNode(ReactBaseTextShadowNode reactBaseTextShadowNode, SpannableStringBuilder spannableStringBuilder, List<SetSpanOperation> list) {
        ...
        if (length2 >= length) {
            ...
            if (Build.VERSION.SDK_INT >= 21 && reactBaseTextShadowNode.mLetterSpacing != Float.NaN) {
