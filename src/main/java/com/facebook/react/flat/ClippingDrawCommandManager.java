...
public abstract class ClippingDrawCommandManager extends DrawCommandManager {
    ...
    /* access modifiers changed from: package-private */
    public abstract boolean regionAboveTouch(int i, float f2, float f3);

    /* access modifiers changed from: package-private */
    public abstract int regionStopIndex(float f2, float f3);
    ...
    private void updateClippingToCurrentRect() {
        ...
        if (!z) {
            ...
            for (int i3 = 0; i3 < size; i3++) {
                this.mFlatViewGroup.removeDetachedView(this.mViewsToRemove.valueAt(i3));
            }
        }
            ...
            if (i4 <= i7) {
                while (i4 != i7) {
                    if (this.mDrawCommands[i4] instanceof DrawView) {
                        ...
                        this.mFlatViewGroup.addViewInLayout((View) a.a(this.mClippedSubviews.get(drawView.reactTag)), i5);
        ...
        while (i4 < this.mStop) {
            if (this.mDrawCommands[i4] instanceof DrawView) {
                ...
                this.mFlatViewGroup.addViewInLayout((View) a.a(this.mClippedSubviews.get(drawView2.reactTag)), i5);
    ...
    public void draw(Canvas canvas) {
        ...
        while (i < this.mStop) {
            ...
            if (drawCommand instanceof DrawView) {
                String str = TAG;
                FLog.w(str, "Unexpected DrawView command at index " + (i4 - 1) + " with mStop=" + this.mStop + ". " + Arrays.toString(this.mDrawCommands));
            } else {
    ...
    @Nullable
    public NodeRegion anyNodeRegionWithinBounds(float f2, float f3) {
        int regionStopIndex = regionStopIndex(f2, f3);
        while (true) {
            ...
            if (regionAboveTouch(i, f2, f3)) {
                break;
            } else if (nodeRegion.withinBounds(f2, f3)) {
                return nodeRegion;
            } else {
                regionStopIndex = i;
            }
    ...
    @Nullable
    public NodeRegion virtualNodeRegionWithinBounds(float f2, float f3) {
        int regionStopIndex = regionStopIndex(f2, f3);
        while (true) {
            int i = regionStopIndex - 1;
            if (regionStopIndex <= 0) {
