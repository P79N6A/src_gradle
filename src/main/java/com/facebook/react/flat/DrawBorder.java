...
public final class DrawBorder extends AbstractDrawBorder {
    ...
    private static int fastBorderCompatibleColorOrZero(float f2, float f3, float f4, float f5, int i, int i2, int i3, int i4) {
        int i5 = -1;
        int i6 = (f2 > 0.0f ? i : -1) & (f3 > 0.0f ? i2 : -1) & (f4 > 0.0f ? i3 : -1);
        if (f5 > 0.0f) {
            i5 = i4;
        }
        int i7 = i5 & i6;
    ...
    private void drawRectangularBorders(Canvas canvas) {
        ...
        if (fastBorderCompatibleColorOrZero == 0) {
            ...
            if (resolveWidth3 > 0.0f) {
                canvas.drawRect(left, top, f4, f3, PAINT);
            }
            if (resolveWidth > 0.0f) {
                canvas.drawRect(f4, top, right, f2, PAINT);
            }
            if (resolveWidth4 > 0.0f) {
                canvas.drawRect(f5, f2, right, bottom, PAINT);
            }
            if (resolveWidth2 > 0.0f) {
                canvas.drawRect(left, f3, f5, bottom, PAINT);
            }
        }
    }
    ...
    public final void setBorderWidth(int i, float f2) {
        if (i != 8) {
            switch (i) {
                case 0:
