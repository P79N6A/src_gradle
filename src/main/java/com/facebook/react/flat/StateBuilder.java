...
public final class StateBuilder {
    ...
    private static float roundToPixel(float f2) {
        return (float) Math.floor((double) (f2 + 0.5f));
    }
    ...
    public final void applyUpdates(FlatShadowNode flatShadowNode) {
        ...
        float f2 = layoutX;
        float f3 = layoutY;
        float f4 = layoutWidth + layoutX;
        float f5 = layoutHeight + layoutY;
        collectStateForMountableNode(flatShadowNode2, f2, f3, f4, f5, Float.NEGATIVE_INFINITY, Float.NEGATIVE_INFINITY, Float.POSITIVE_INFINITY, Float.POSITIVE_INFINITY);
        updateViewBounds(flatShadowNode2, f2, f3, f4, f5);
    }
    ...
    private void updateViewBounds(FlatShadowNode flatShadowNode, float f2, float f3, float f4, float f5) {
    ...
    private void addNodeRegion(FlatShadowNode flatShadowNode, float f2, float f3, float f4, float f5, boolean z) {
        if (f2 != f4 && f3 != f5) {
            flatShadowNode.updateNodeRegion(f2, f3, f4, f5, z);
    ...
    private boolean processNodeAndCollectState(FlatShadowNode flatShadowNode, float f2, float f3, float f4, float f5, float f6, float f7, boolean z, boolean z2) {
        float layoutWidth = flatShadowNode.getLayoutWidth();
        float layoutHeight = flatShadowNode.getLayoutHeight();
        float layoutX = f2 + flatShadowNode.getLayoutX();
        float layoutY = f3 + flatShadowNode.getLayoutY();
        float f8 = layoutWidth + layoutX;
        float f9 = layoutHeight + layoutY;
        boolean mountsToView = flatShadowNode.mountsToView();
        if (!z) {
            addNodeRegion(flatShadowNode, layoutX, layoutY, f8, f9, !mountsToView);
        }
        if (mountsToView) {
            ensureBackingViewIsCreated(flatShadowNode);
            addNativeChild(flatShadowNode);
            boolean collectStateForMountableNode = collectStateForMountableNode(flatShadowNode, 0.0f, 0.0f, f8 - layoutX, f9 - layoutY, f4 - layoutX, f5 - layoutY, f6 - layoutX, f7 - layoutY);
            if (!z) {
                this.mDrawCommands.add(flatShadowNode.collectDrawView(layoutX, layoutY, f8, f9, f4, f5, f6, f7));
            }
            if (z2) {
                return collectStateForMountableNode;
            }
            updateViewBounds(flatShadowNode, layoutX, layoutY, f8, f9);
            return collectStateForMountableNode;
        }
        return collectStateRecursively(flatShadowNode, layoutX, layoutY, f8, f9, f4, f5, f6, f7, false, false);
    }

    private boolean collectStateForMountableNode(FlatShadowNode flatShadowNode, float f2, float f3, float f4, float f5, float f6, float f7, float f8, float f9) {
        boolean z;
        boolean z2;
        float f10;
        boolean z3;
        ...
        float f17 = f9;
        boolean z6 = false;
        if (flatShadowNode.hasNewLayout() || flatShadowNode.isUpdated() || flatShadowNode.hasUnseenUpdates() || flatShadowNode2.clipBoundsChanged(f14, f15, f16, f17)) {
            z = true;
        } else {
            z = false;
        }
        ...
        flatShadowNode2.setClipBounds(f14, f15, f16, f17);
        ...
        if (!z3 && flatShadowNode.isVirtualAnchor()) {
            addNodeRegion(flatShadowNode, f2, f3, f4, f5, true);
        }
        boolean collectStateRecursively = collectStateRecursively(flatShadowNode, f2, f3, f4, f5, f13, f12, f11, f10, z3, z2);
    ...
    private boolean collectStateRecursively(com.facebook.react.flat.FlatShadowNode r26, float r27, float r28, float r29, float r30, float r31, float r32, float r33, float r34, boolean r35, boolean r36) {
        ...
        L_0x00a5:
            ...
            com.facebook.react.uimanager.ReactShadowNodeImpl r3 = r10.getChildAt((int) r1)
