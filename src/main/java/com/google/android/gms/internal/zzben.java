...
public final class zzben extends Fragment implements zzbds {
    ...
    public static zzben zza(FragmentActivity fragmentActivity) {
        ...
        if (weakReference != null) {
            zzben zzben = (zzben) weakReference.get();
            if (zzben != null) {
                return zzben;
            }
        }
        try {
            zzben zzben2 = (zzben) fragmentActivity.getSupportFragmentManager().findFragmentByTag("SupportLifecycleFragmentImpl");
            if (zzben2 == null || zzben2.isRemoving()) {
                zzben2 = new zzben();
                fragmentActivity.getSupportFragmentManager().beginTransaction().add((Fragment) zzben2, "SupportLifecycleFragmentImpl").commitAllowingStateLoss();
            }
            zzaEH.put(fragmentActivity, new WeakReference(zzben2));
            return zzben2;
