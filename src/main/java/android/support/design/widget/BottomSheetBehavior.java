...
public class BottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    public class _lancet {
        ...
        @Insert
        @TargetClass
        static float com_ss_android_ugc_aweme_lancet_DesignBugFixLancet_getYVelocity(BottomSheetBehavior bottomSheetBehavior) {
            try {
                return bottomSheetBehavior.getYVelocity$___twin___();
        ...
        @Insert
        @TargetClass
        static boolean com_ss_android_ugc_aweme_lancet_DesignBugFixLancet_onTouchEvent(BottomSheetBehavior bottomSheetBehavior, CoordinatorLayout coordinatorLayout, View view, MotionEvent motionEvent) {
            try {
                return bottomSheetBehavior.onTouchEvent$___twin___(coordinatorLayout, view, motionEvent);
            } catch (Exception unused) {
                return false;
            }
        }
    ...
    private float getYVelocity() {
        return _lancet.com_ss_android_ugc_aweme_lancet_DesignBugFixLancet_getYVelocity(this);
    }

    public boolean onTouchEvent(CoordinatorLayout coordinatorLayout, View view, MotionEvent motionEvent) {
        return _lancet.com_ss_android_ugc_aweme_lancet_DesignBugFixLancet_onTouchEvent(this, coordinatorLayout, view, motionEvent);
    }
    ...
    public float getYVelocity$___twin___() {
        if (this.velocityTracker == null) {
            return 0.0f;
        }
        this.velocityTracker.computeCurrentVelocity(PushConstants.PUSHSERVICE_INFO_SENDMESSAGE_BY_NOTIFICATIONSERVICE, this.maximumVelocity);
    ...
    public boolean shouldHide(View view, float f2) {
        ...
        if (view.getTop() >= this.collapsedOffset && Math.abs((((float) view.getTop()) + (f2 * 0.1f)) - ((float) this.collapsedOffset)) / ((float) this.peekHeight) > 0.5f) {
            return true;
        }
        return false;
    }
    ...
    public boolean onTouchEvent$___twin___(CoordinatorLayout coordinatorLayout, V v, MotionEvent motionEvent) {
        if (!v.isShown()) {
            return false;
        }
    ...
    public boolean onLayoutChild(CoordinatorLayout coordinatorLayout, V v, int i) {
        ...
        if (this.peekHeightAuto) {
            if (this.peekHeightMin == 0) {
                this.peekHeightMin = coordinatorLayout.getResources().getDimensionPixelSize(C0906R.dimen.ea);
            }
    ...
    public boolean onNestedPreFling(@NonNull CoordinatorLayout coordinatorLayout, @NonNull V v, @NonNull View view, float f2, float f3) {
        if (view != this.nestedScrollingChildRef.get() || (this.state == 3 && !super.onNestedPreFling(coordinatorLayout, v, view, f2, f3))) {
            return false;
        }
        return true;
    }
