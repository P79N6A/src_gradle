...
public class TiledPagedList<T> extends PagedList<T> implements PagedStorage.Callback {
            ...
            if (pageResult.isInvalid()) {
                ...
                if (i == 0 || i == 3) {
                    ...
                    if (TiledPagedList.this.mBoundaryCallback != null) {
                        boolean z3 = false;
                        if (TiledPagedList.this.mStorage.size() == 0) {
                            z = true;
                        } else {
                            z = false;
                        }
                        if (!z && pageResult.leadingNulls == 0 && pageResult.positionOffset == 0) {
                            z2 = true;
                        } else {
                            z2 = false;
                        }
                        int size = TiledPagedList.this.size();
                        if (!z && ((i == 0 && pageResult.trailingNulls == 0) || (i == 3 && pageResult.positionOffset + TiledPagedList.this.mConfig.pageSize >= size))) {
                            z3 = true;
                        }
                        TiledPagedList.this.deferBoundaryCallbacks(z, z2, z3);
                    }
                    return;
                }
    ...
    @WorkerThread
    TiledPagedList(@NonNull PositionalDataSource<T> positionalDataSource, @NonNull Executor executor, @NonNull Executor executor2, @Nullable PagedList.BoundaryCallback<T> boundaryCallback, @NonNull PagedList.Config config, int i) {
